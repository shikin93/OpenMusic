...
const { mapDBToModel } = require('../../utils/DBToModel');
...
class MusicService {
  ...
  async editSongById(id, { title, year, performer, genre, duration }) {
    ...
    const query = {
      text: 'UPDATE music SET title = $1, year = $2, performer = $3, genre = $4, duration = $5, updated = $6 WHERE id = $7 RETURNING id',
  ...
  async deleteSongById(id) {
    ...
    const result = await this._pool.query(query);
    if (!result.rowCount) {
      throw new NotFoundError('Tidak dapat menghapus lagu. id tidak ditemukan');
    }
  }
}
